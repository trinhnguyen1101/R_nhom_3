...
# Đếm từ khóa thường gặp
keywords <- c("ung ho", "ủng hộ", "bao", "bão", "yagi", "dong bao", "đồng bào", "mien trung", "góp", "gop","tap the", "tập thể","cong ty","nhom","to chuc")
keyword_freq <- sapply(keywords, function(k) sum(grepl(k, all$desc_clean, fixed = TRUE)))
cat("Tần suất xuất hiện các từ khóa phổ biến trong nội dung chuyển khoản:\n")
print(keyword_freq)
all$desc_clean <- NULL
```
...
Để đảm bảo phân cụm hiệu quả, dữ liệu đã được loại bỏ các ngoại lệ bằng phương pháp z-score trước đó. Chúng tôi chỉ sử dụng cột money từ dữ liệu đã làm sạch (all_clean) để phân cụm. 
...
Mặc dù tại K=9 biểu đồ cho thấy WSS tiếp tục giảm và có thể là một lựa chọn tốt về mặt toán học, nhưng trong thực tế, việc phân loại các giao dịch theo giá trị tiền thành quá nhiều nhóm có thể dẫn đến việc phân cấp quá chi tiết, gây khó khăn trong việc phân tích và đưa ra quyết định.
...
```
Biểu đồ cho thấy dữ liệu được chia thành 4 cụm rõ rệt, với cụm số 2 (màu xanh lá) đại diện cho các giao dịch có giá trị cao nhất, trong khi cụm số 1 và 4 chứa phần lớn các giao dịch nhỏ.
...
```
Nhận xét
  Cụm 1 và 4 chiếm phần lớn giao dịch lần lượt là 67,57% và 26,96% nhưng với giá trị nhỏ và trung bình (dưới 2 triệu đồng).
  
  Cụm 2 tuy có số lượng rất nhỏ 0.82%, nhưng chiếm phần lớn giá trị 16,34% tổng tiền nhờ các giao dịch lớn (từ 8 triệu đến 42 triệu).
  
  Cụm 3 nằm giữa cụm 2 và 4, đại diện cho các giao dịch khá.
  
  Ngoài 4 cụm trên thì các cụm ngoại lệ được bỏ ra từ Z-score được xếp vào nhóm giao dịch rất lớn, với tỉ lệ tổng số giao dịch và tỉ lệ tổng tiền giao dịch lần lượt 0.12% và 21.81%.

Việc phân cụm này hỗ trợ việc phân tích hành vi ủng hộ, giúp dễ dàng nhận diện nhóm nhà hảo tâm có đóng góp lớn để có thể tri ân. Đồng thời, nó cũng giúp phân loại dữ liệu cho các mô hình dự đoán hoặc phân tích nâng cao về sau.
